...
router.get('/members/:id', async (req, res, next) => {
  try {
    ...
    if (!item) throw new AppError(404, 'Team member not found');
    res.json(item);
  } catch (e) { next(e); }
});
...
router.get('/org-nodes', async (req, res, next) => {
  try {
    ...
    const buildTree = (parentId: string | null = null): any[] =>
      items.filter(n => n.parentId === parentId).sort((a, b) => a.displayOrder - b.displayOrder).map(n => ({
        ...n,
...
const REORDER_MODELS = ['leadership', 'departments', 'members', 'org-nodes', 'founders', 'advisors', 'statistics', 'awards', 'testimonials', 'locations', 'custom-sections'] as const;
...
router.patch('/:resource/reorder', authenticate, authorize('ADMIN'), async (req, res, next) => {
  try {
    ...
    if (!modelName) throw new AppError(400, `Invalid resource: ${resource}`);
    if (!REORDER_MODELS.includes(resource as any)) throw new AppError(400, `Reorder not supported for ${resource}`);
    ...
    await prisma.$transaction(
      items.map((item: { id: string; displayOrder: number }) =>
        model[modelName].update({ where: { id: item.id }, data: { displayOrder: item.displayOrder } })
      )
    );
...
const TOGGLE_MODELS = ['leadership', 'departments', 'members', 'custom-sections'] as const;
...
router.patch('/:resource/:id/toggle', authenticate, authorize('ADMIN'), async (req, res, next) => {
  try {
    ...
    const modelName = prismaModelMap[resource];
    if (!modelName) throw new AppError(400, `Invalid resource: ${resource}`);
    if (!TOGGLE_MODELS.includes(resource as any)) throw new AppError(400, `Toggle not supported for ${resource}`);
...
const BULK_DELETE_MODELS = ['leadership', 'departments', 'members', 'org-nodes', 'founders', 'advisors', 'statistics', 'awards', 'testimonials', 'locations', 'custom-sections', 'media'] as const;
...
router.post('/:resource/bulk-delete', authenticate, authorize('ADMIN'), async (req, res, next) => {
  try {
    ...
    const modelName = prismaModelMap[resource];
    if (!modelName) throw new AppError(400, `Invalid resource: ${resource}`);
    if (!BULK_DELETE_MODELS.includes(resource as any)) throw new AppError(400, `Bulk delete not supported for ${resource}`);
