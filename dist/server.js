...
let docBotPort = 8000;
let docBotProcess = null;
let pgChatbotPort = 8010;
let pgChatbotProcess = null;
...
async function findFreePort(startPort, label) {
...
async function startDocBot() {
    ...
    proc.on('exit', (code) => {
        if (code !== 0)
            console.warn(`[DocBot] Exited with code ${code}`);
        if (pgChatbotProcess === proc)
...
async function startPgChatbot() {
    ...
    proc.on('exit', (code) => {
        if (code !== 0)
            console.warn(`[PGChatbot] Exited with code ${code}`);
        if (pgChatbotProcess === proc)
            pgChatbotProcess = null;
    });
...
async function start() {
    try {
        ...
        try {
            ...
            console.warn('[Chatbot] Initial Q&A build skipped:', err.message);
        }
        ...
        if (pgChatbot.status === 'fulfilled' && pgChatbot.value) {
            ...
            try {
                ...
                console.warn('[SyncPG] Initial sync failed:', err.message);
            }
        }
        // Admin endpoints
