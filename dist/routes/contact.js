...
async function sendThankYou(name, email, message) {
...
router.post('/', publicLimiter, validate(contactSchema), async (req, res, next) => {
    try {
        ...
        sendThankYou(name, email, sanitizedMessage).catch(err => console.error('[Contact] Thank-you email failed:', err.message));
