...
import { createCampaign, sendBatch } from '../services/mail/batch.service.js';
...
router.post('/send-direct', async (req, res, next) => {
    try {
        ...
        if (!isConnected)
            throw new AppError(502, 'SMTP transport is not connected');
...
router.post('/campaigns/:id/send', async (req, res, next) => {
    try {
        ...
        if (campaign.status === 'SENDING')
            throw new AppError(400, 'Campaign is already sending');
        const isConnected = await verifyTransport();
        if (!isConnected)
            throw new AppError(502, 'SMTP transport is not connected');
        let recipients = [];
        ...
        if (campaign.template) {
            ...
            if (!template)
                throw new AppError(400, 'No template found and campaign has no bodyHtml');
