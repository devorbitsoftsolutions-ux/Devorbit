...
import { renderAndSend } from './email.service.js';
...
export async function sendBatch(options) {
    const { campaignId, subjectTemplate, bodyTemplate, recipients, onProgress, } = options;
    ...
    for (let i = 0; i < recipients.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(batch.map(recipient => renderAndSend(recipient.email, bodyTemplate, subjectTemplate, { name: recipient.data?.name || recipient.name || '', email: recipient.email, ...recipient.data }, {
            campaignId,
            recipientId: recipient.recipientId,
            recipientType: recipient.recipientType,
            messageType: recipient.data?.messageType,
            reason: recipient.data?.reason,
        })));
