...
function generateServiceQA(items) {
    const pairs = [];
...
function generatePortfolioQA(items) {
    const pairs = [];
...
function generateInitiativeQA(items) {
    const pairs = [];
...
function generateConsultingQA(items) {
    const pairs = [];
...
function generateSiteInfoQA(settings) {
    const pairs = [];
    ...
    if (jobs.length > 0) {
        ...
        for (const j of jobs) {
            ...
            if (j.requirements.length > 0)
                lines.push(`Requirements: ${j.requirements.join(', ')}`);
            if (j.benefits.length > 0)
                lines.push(`Benefits: ${j.benefits.join(', ')}`);
            lines.push('');
        }
    }
...
//# sourceMappingURL=chatbot-sync.js.map